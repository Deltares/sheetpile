...
def set_up_and_train_2d_model():
    directory_inputs = "D:\sheetpile\src\\ai_model\inputs"
    ...
    for pickle_name in pickle_names:
        feature_names, all_data = load_from_pickle(os.path.join(directory_inputs, pickle_name))
        global_data.append(all_data)
    dataset_all = np.concatenate(global_data, axis=0)
    # find indexes for input
    inputs_lookup = ['FRICTION_ANGLE', 'geometry', 'water_pressure_stage_3', 'YOUNGS_MODULUS']
    ...
    # create zero array for the inputs
    inputs_dataset_update = np.zeros((len(inputs_lookup), inputs_dataset.shape[0], inputs_dataset.shape[2], inputs_dataset.shape[3]))
    outputs_dataset_update = np.zeros((outputs_dataset.shape[0], outputs_dataset.shape[2], outputs_dataset.shape[3]))
    for counter, input in enumerate(range(inputs_dataset.shape[0])):
        inputs_dataset_update[:, counter, :, :] = inputs_dataset[counter, :, :, :]
    for counter, output in enumerate(range(inputs_dataset.shape[0])):
        outputs_dataset_update[counter :, :] = outputs_dataset[counter, :, :, :]

    # split the data into train and test
    percentage_train = 0.8
    number_of_inputs = inputs_dataset_update.shape[1]
    train_input_dataset = inputs_dataset_update[:, int(percentage_train * number_of_inputs):, :, :]
    train_output_dataset = outputs_dataset_update[int(percentage_train * number_of_inputs):]
    test_input_dataset = inputs_dataset_update[:, int(percentage_train * number_of_inputs):, :, :]
    test_output_dataset = outputs_dataset_update[int(percentage_train * number_of_inputs):, :, :]
...
def Generator_multi_input(input_shape=(256, 256, 1)):
    ...
    outputs = tf.keras.layers.Concatenate()([y1, y2, y3, y4])
...
if __name__ == "__main__":
    ...
    checkpoint_dir = "D:\sheetpile\src\\ai_model\\training_checkpoints_2d"
